...
def _candidate(name: str, priority: int, growth_score: int, *, series: bool = False, scope: str = "science_future") -> dict:
    source_context = {
        "fallback_editorial_answers": {
            "what_happened": "x", "why_happening": "x", "who_is_affected": "x", "personal_impact": "x", "action_now": "x"
        }
    }
    if series:
    ...
    return {
        ...
        "trend_scope": scope,
        "score": 90,
        "growth_v2": {"growth_score": growth_score, "binding_score": 80, "priority_score": priority, "series_candidate": series},
...
def test_series_is_not_forced_when_five_stronger_fresh_candidates_exist() -> None:
    items = [
        _candidate("series", 81, 80, series=True),
        ...
        _candidate("e", 92, 90, scope="sports"),
    ]
...
def test_series_can_win_when_it_is_competitive() -> None:
    items = [
        _candidate("series", 99, 96, series=True),
        _candidate("a", 95, 93, scope="world_society"),
        _candidate("b", 94, 92, scope="technology_ai"),
        _candidate("c", 93, 91, scope="business_economy"),
        _candidate("d", 92, 90, scope="mobility_energy"),
        _candidate("e", 91, 89, scope="sports"),
    ]
    selected = growth.diversify(items, "evening")
